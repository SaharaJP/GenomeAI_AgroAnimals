...
def boundary_decision_intelligence(
    ...
    top_actions_map: dict[str, int] = {}
    linked_alerts = 0
    for row in decision_rows:
        action = str(row.get('action') or '').strip() or 'unknown'
        top_actions_map[action] = top_actions_map.get(action, 0) + 1
        if row.get('related_alert'):
            linked_alerts += 1
    top_actions = [
        DecisionIntelligenceTopAction(action=action, count=count)
        for action, count in sorted(top_actions_map.items(), key=lambda kv: (-kv[1], kv[0]))[:5]
    ]
    latest_decisions = [_map_decision(row, request=request) for row in decision_rows[: int(limit)]]
    summary = DecisionIntelligenceSummary(
        total_decisions=int(decisions_payload.get('total') or len(decision_rows)),
        accepted_feedback=int(metrics_raw.get('accepted_count') or 0),
        rejected_feedback=int(metrics_raw.get('rejected_count') or 0),
        acceptance_rate=float(metrics_raw.get('acceptance_rate') or 0.0),
        linked_alerts=linked_alerts,
    )
    return DecisionIntelligenceResponse(summary=summary, top_actions=top_actions, latest_decisions=latest_decisions)
