...
- Кривая надоя — Wood (1967) с per-cow подгонкой по milk_yields (≥30 in-lactation записей);
  при недостатке данных — Holstein-defaults a=25, b=0.20, c=0.003.
- Базовая вероятность выбытия — стратифицирована по паритету (L1=1.8%, L2-3=2.0%,
  L4=2.5%, L5+=3.5% в месяц; Compton 2017).
- Композитный health-score объединяет 8 сигналов: mastitis, late-DIM, parity (компенсация
  снижения продуктивности), хронический SCC, lameness, возраст (>5 лет), days-open
  (затянувшаяся открытость), treatment recurrence (рецидив лечения в 60 дн.).
- Многолактационная динамика моделируется как циклический сброс DIM (305 дойных + 60 сухостоя).
