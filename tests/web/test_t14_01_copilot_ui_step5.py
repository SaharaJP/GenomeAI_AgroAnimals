...
def test_copilot_target_api_includes_detail_actions(client: TestClient):
    ...
    payload = resp.json()
    actions = payload["detail_actions"]
    hrefs = [row["href"] for row in actions]
    assert "/jobs?q=kpi_run_001" in hrefs
    assert "/reports?dv=dv_demo" in hrefs
    assert any(h.endswith("#table-preview") for h in hrefs)
    assert any(h.startswith("/api/copilot/fact?") for h in hrefs)


def test_copilot_target_page_renders_quick_action_links(client: TestClient):
    artifacts_root = Path(os.environ["GENOMEAI_ARTIFACTS_ROOT"])
    _seed_kpi_artifacts(artifacts_root)
    _login(client)
    ...
    assert resp.status_code == 200
    assert "Быстрые переходы" in resp.text
    assert "/jobs?q=kpi_run_001" in resp.text
    assert "#table-preview" in resp.text
    assert "Артефакты/источники" in resp.text
